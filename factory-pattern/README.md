### Instead of using Supplier for invoking constructor, we can use map based factory or switch case too
```java  
    private static final Map<NotificationType, Supplier<INotification>> FACTORY_MAP;
    static {
        FACTORY_MAP = new EnumMap<>(NotificationType.class);
        FACTORY_MAP.put(NotificationType.EMAIL, EmailNotification::new);
        FACTORY_MAP.put(NotificationType.SMS, SMSNotification::new);
    }
```
